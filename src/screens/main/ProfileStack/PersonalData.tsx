...
const PersonalData = ({navigation}: any) => {
      const { t } = useTranslation();
          ...
          const employeeData = useSelector((state: any) => state.getEmployeePersonalData);
            ...
            const validateForm = () => {
              ...
              if (!formValues.image.uri) newErrors.image = t('Image is required');
              if (!formValues.firstName.trim()) newErrors.firstName = t('First name is required');
              if (!formValues.lastName.trim()) newErrors.lastName = t('Last name is required');
              if (!formValues.dateOfBirth) newErrors.dateOfBirth = t('Date of birth is required');
              if (!formValues.position.trim()) newErrors.position = t('Position is required');
              if (!formValues.country.trim()) newErrors.country = t('Country is required');
              if (!formValues.state.trim()) newErrors.state = t('State is required');
              if (!formValues.city.trim()) newErrors.city = t('City is required');
              if (!formValues.fullAddress.trim()) newErrors.fullAddress = t('Full address is required');
  ...
  return (
    <View style={{flex: 1}}>
      <NormalHeader onPress={() => navigation.goBack()} title={t("Personal Data")} />

    <ScrollView contentContainerStyle={{flexGrow: 1}}>
      <View style={{padding: responsiveWidth(4)}}>
            <WhiteContainers paddingHorizontal={5}>
                            <BoldText title={t('My Personal Data')} mrgnTop={1} txtColour={APPCOLORS.BLACK} fntWeight='bold' fontSize={2}/>
                            <BoldText title={t('Details about my personal data')} txtColour={APPCOLORS.DARK_GRAY} fontSize={1.7}/>
                ...
                <View style={{alignItems: 'center', marginTop: responsiveHeight(2)}}>
                    ...
                    <Image source={selectedImage || employeeData.personalData.profileImage ? {uri: selectedImage || `${baseUrl}/${employeeData.personalData.profileImage}`} : AppImages.pfps} style={{width: 200, height: 200, borderRadius: 25}} />
                        ...
                        <BoldText title={selectedImage ? t('Change Photo') : t('Upload Photo')} textAligm={'center'} txtColour={APPCOLORS.DARK_GRAY} fntWeight='bold' fontSize={2} mrgnTop={2}/>
                    </TouchableOpacity>
                <View style={{marginHorizontal: responsiveWidth(10)}}>
                <BoldText title={t('Format should be in .jpeg .png atleast 800x800px and less than 5MB')} mrgnTop={0.5} textAligm={'center'} txtColour={APPCOLORS.DARK_GRAY} fontSize={1.5}/>
                </View>
                </View>

            <View>
                <View style={{marginTop: responsiveHeight(2)}}>
                <NormalInput labelTitle={t('First Name')} icon='person' placeholder={t('First Name')} value={formValues.firstName}
                ...
                <NormalInput labelTitle={t('Last Name')} icon='person' placeholder={t('Last Name')} value={formValues.lastName}
                            onChangeText={(text: any) => {
                ...
                <NormalInput labelTitle={t('Date Of Birth')} icon='calendar' placeholder={t('Date Of Birth')} 
              ...
              />
                ...
                <View style={{marginTop: responsiveHeight(1.5)}}>
                <NormalInput labelTitle={t('Position')} icon='calendar' placeholder={t('Position')} value={formValues.position}
                            onChangeText={(text: any) => {
    ...
    <BoldText title={t('Address')} mrgnTop={1} txtColour={APPCOLORS.BLACK} fntWeight='bold' fontSize={2}/>
    <BoldText title={t('Your current domicile')} txtColour={APPCOLORS.DARK_GRAY} fontSize={1.7}/>

    <View style={{marginTop: responsiveHeight(1.5)}}>
    <NormalInput labelTitle={t('Country')} icon='location' placeholder={t('Country')} value={formValues.country}
                            onChangeText={(text: any) => {
                ...
                <View style={{marginTop: responsiveHeight(1.5)}}>
                <NormalInput labelTitle={t('State')} icon='location' placeholder={t('State')} value={formValues.state}
                            onChangeText={(text: any) => {
                ...
                <View style={{marginTop: responsiveHeight(1.5)}}>
                <NormalInput labelTitle={t('City')} icon='location' placeholder={t('City')} value={formValues.city}
                            onChangeText={(text: any) => {
    ...
    <View>
    <BoldText title={t('Full Address')} txtColour={APPCOLORS.DARK_GRAY} fontSize={2} mrgnTop={2}/>
      <TextInput
    ...
    <UpdateProfileModal 
    isModalVisible={isUpdateModalVisible} imageSource={AppImages.personBox} yesBtnDisabled={loading} yesBtnTitle={loading ? "Waiting..." : t('Yes, Update Profile')} noBtnTitle={t('No, Let me check')} yesBtnOnPress={() => handleUpdateProfile()} noBtnOnPress={() => setIsUpdateModalVisible(false)}   title={t("Update Profile")} subTitle={t("Are you sure you want to update your profile? This will help us improve your experience and provide personalized features.")}
    />


<UpdateProfileModal 
    isModalVisible={isSuccessModalVisible} imageSource={AppImages.personBox} yesBtnTitle={t('View My Profile')} yesBtnOnPress={() => {
        setIsSuccessModalVisible(false);
        dispatch(isUpdatedFalseAction())
          }} title={t("Profile Updated!")} subTitle={t("Your profile has been successfully updated. We’re excited to see you take this step!")}
    />
    </View>
  )
}
